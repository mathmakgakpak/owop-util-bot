...
class Screenshot {
			...
			for (let xx = x1; xx <= x2; xx++) {
				for (let yy = y1; yy <= y2; yy++) {
					/*let wb = Client.options.worldBorder;
           			if (!that.clientOptions.unsafe && (x > wb || y > wb || x < ~wb || y < ~wb)) continue;*/ // fix it later
					while (this.bot.ws.readyState !== 1) await sleep(1000);

					if (this.bot.chunkSystem.getChunk(xx, yy)) { //is this really faster :thinking:
						chunksLasted--;
	...
	putOnCanvas(x1, y1, x2, y2) {
		...
		for (let canvasX = -Math.abs(absMod(x1, 16)), xx = x1c; xx <= x2c; xx++, canvasX += 16) { // TO-DO fix that
	...
	takeScreenshotRadius(radius = 10, x = 0, y = 0) {
		let x1 = x - radius;
		let y1 = y - radius;
		let x2 = x + radius;
		let y2 = y + radius;
		...
		return this.takeScreenshot(x1, y1, x2, y2);
	}
}
